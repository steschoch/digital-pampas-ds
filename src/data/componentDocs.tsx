...
const PORTAL_DOCS: PortalDocCfg[] = [
  ...
  },
  {
    ...
    anatomy: [{ id: 1, label: 'Grid + axes', desc: 'Subtle grid, mono labels.' }, { id: 2, label: 'Series', desc: '1–3 lines, token colors.' }, { id: 3, label: 'Data table', desc: 'Visually-hidden, for screen readers.' }],
    prompt: '## LineChart\nProps: series {label,colorVar,points{date,value}[]}[], height?, yFormat?, showGrid?, showDots?. Pure SVG, no lib.',
    code: `<LineChart series={series} yFormat={fmt} />`,
    ...
    guidelines: [{ type: 'do', text: 'Limit to 3 series for legibility.' }, { type: 'dont', text: 'Plot unaligned x-axes across series.' }],
  },
  {
  ...
  },
  {
    ...
    anatomy: [{ id: 1, label: 'Label', desc: 'Caption, muted.' }, { id: 2, label: 'Value', desc: 'Big display number.' }, { id: 3, label: 'Delta', desc: '↑/↓ + %, green/red by good/bad.' }, { id: 4, label: 'Slot', desc: 'Optional Sparkline/badge.' }],
    prompt: '## StatCard\nProps: label, value, delta? {value,direction,positiveIsGood?}, badge?, children? (sparkline), loading?.',
    code: `<StatCard label="Reply rate" value="8.95%" delta={{ value: '2.1%', direction: 'up' }}><Sparkline points={pts} /></StatCard>`,
    ...
    guidelines: [{ type: 'do', text: 'Set positiveIsGood so color matches meaning (bounce down = good).' }, { type: 'dont', text: 'Cram more than one KPI per card.' }],
  },
  {
  ...
  },
  {
    ...
    description: 'Data-driven phase tracker. Horizontal/compact for overviews, vertical/detailed for detail views.',
    live: <LivePreview h={160} scale={1}><div style={{ padding: 24 }}><Timeline phases={P_TIMELINE} orientation="horizontal" density="compact" /></div></LivePreview>,
    anatomy: [{ id: 1, label: 'Node', desc: 'done ✓ / active (pulsing) / upcoming (dotted) / delayed (amber).' }, { id: 2, label: 'Connector', desc: 'Line between nodes; filled when done.' }, { id: 3, label: 'Detail', desc: 'Vertical density: dates + deliverables.' }],
    prompt: '## Timeline\nProps: phases {id,label,description?,status,plannedStart?,plannedEnd?,startedAt?,completedAt?,deliverables?[]}[], orientation, density.',
    code: `<Timeline phases={phases} orientation="horizontal" density="compact" />`,
    ...
    a11y: [{ rule: 'Ordered list; status is text', status: 'pass' }, { rule: 'Active pulse respects reduced-motion', status: 'pass' }],
    guidelines: [{ type: 'do', text: 'Use compact on overview, detailed on the dossier.' }, { type: 'dont', text: 'Encode status with color only.' }],
  },
  {
